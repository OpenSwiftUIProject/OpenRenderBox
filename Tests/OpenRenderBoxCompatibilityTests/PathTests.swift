...
struct PathTests {
    ...
    func createEllipsePath() {
        ...
        let path = RBPath(ellipseIn: rect, transform: nil)
        path.release()
    }
    ...
    func createRoundedRectPath() {
        let rect = CGRect(x: 0, y: 0, width: 100, height: 100)
        let path = RBPath(roundedRect: rect, cornerWidth: 10, cornerHeight: 10, style: .circular, transform: nil)
        path.release()
    }
    ...
    func createUnevenRoundedRectPath() {
        let rect = CGRect(x: 0, y: 0, width: 100, height: 100)
        let path = RBPath(roundedRect: rect, topLeftRadius: 5, bottomLeftRadius: 10, bottomRightRadius: 15, topRightRadius: 20, style: .continuous, transform: nil)
        path.release()
    }
    ...
    func pathEqualToPath() {
        let rect = CGRect(x: 0, y: 0, width: 100, height: 100)
        let path1 = RBPath(rect: rect, transform: nil)
        let path2 = RBPath(rect: rect, transform: nil)
        #expect(path1.isEqual(to: path2) == true)
        path1.release()
        path2.release()

        let differentRect = CGRect(x: 0, y: 0, width: 200, height: 200)
        let path3 = RBPath(rect: rect, transform: nil)
        let path4 = RBPath(rect: differentRect, transform: nil)
